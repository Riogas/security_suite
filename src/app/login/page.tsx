...
import { Moon, Sun, Loader2, Lock, LockOpen, ShieldBan, Eye, EyeOff } from "lucide-react";
...
export default function LoginPage() {
  ...
  const [lockState, setLockState] = useState<"idle" | "locked" | "unlocked" | "denied">("idle");
  ...
  const renderLockModal = () => (
    (lockState === "locked" || lockState === "unlocked" || lockState === "denied") && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition-all">
        <div className="bg-card/90 rounded-2xl shadow-2xl p-8 flex flex-col items-center animate-fade-in">
          {lockState === "locked" && <Lock className="w-16 h-16 text-red-400 animate-pulse mb-4" />}
          {lockState === "unlocked" && <LockOpen className="w-16 h-16 text-green-400 animate-bounce mb-4" />}
          {lockState === "denied" && <ShieldBan className="w-16 h-16 text-red-600 animate-shake mb-4" />}
          <span className="text-lg font-semibold">
            {lockState === "locked" ? "Validando credenciales..." :
             lockState === "unlocked" ? "¡Acceso concedido!" :
             lockState === "denied" ? "Acceso denegado" : ""}
          </span>
        </div>
      </div>
    )
  ...
  return (
    <main className="min-h-screen flex items-center justify-center text-foreground px-4 relative">
      <div className="absolute inset-0 w-full h-full -z-10 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url('/fondosecure.png')" }} />
      <div className="absolute inset-0 w-full h-full -z-5 bg-black/30" />

      <button onClick={toggleTheme} className="absolute top-4 right-4 text-muted-foreground hover:text-foreground transition" aria-label="Cambiar tema">
      ...
      >
        <div className="text-center mb-6">
          <div className="flex justify-center">
            <Image src="/logo.png" alt="Logo" width={350} height={350} className="object-contain" priority />
          </div>
        </div>
