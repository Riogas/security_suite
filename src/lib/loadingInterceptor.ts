...
const getLoadingMessage = (url: string, method: string = 'GET'): string => {
  ...
  if (url.includes('listarFuncionalidades')) return "Cargando funcionalidades...";
  ...
  switch (method.toUpperCase()) {
    case 'POST': return "Creando...";
    case 'PUT': return "Actualizando...";
    case 'DELETE': return "Eliminando...";
    case 'GET': return "Cargando datos...";
...
export const setupLoadingInterceptors = (axiosInstance: any) => {
  ...
  axiosInstance.interceptors.request.use(
    (config: InternalAxiosRequestConfig) => {
      ...
      if (shouldShowLoading(url) && loadingProvider && !activeRequests.has(requestKey)) {
