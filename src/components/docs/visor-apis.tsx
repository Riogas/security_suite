...
export function VisorApis({ catalogo }: { catalogo: CatalogoDocs }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Buscador y filtros ── */}
      <div className="sticky top-0 z-20 -mx-1 rounded-xl border bg-background/95 px-3 py-3 backdrop-blur supports-[backdrop-filter]:bg-background/80">
                ...
                <li key={m.nombre} className="shrink-0 lg:shrink">
                  ...
                  >
                    ...
                    <span className="flex shrink-0 items-center gap-1.5">
                      ...
                      <span className="font-mono text-xs tabular-nums opacity-70">{m.total}</span>
                    </span>
                  </button>
                </li>
              ))}
            ...
            <section key={nombreModulo} aria-labelledby={`modulo-${nombreModulo}`}>
              ...
              <div className="divide-y overflow-hidden rounded-xl border bg-card">
                {endpoints.map((e) => {
                  ...
                  return (
                    <article key={e.id} id={ancla(e.id)} className="scroll-mt-32">
                      <button
                        type="button"
            ...
            <section className="rounded-xl border border-dashed p-4">
              ...
              <ul className="mt-2 space-y-2 text-xs text-muted-foreground">
                {catalogo.colapsados.map((c) => (
                  <li key={c.entrada}>
                    <span className="font-mono text-foreground">{c.entrada}</span> — colapsado:{" "}
                    {c.motivo}
                  </li>
                ))}
