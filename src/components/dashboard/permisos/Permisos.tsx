...
import { apiPermisos } from "@/services/api";
...
export default function PermisosTable() {
  ...
  const fetcher = async (opts: { FiltroTexto: string; Estado: string; sinMigrar: boolean; Pagesize: number; CurrentPage: number; signal?: AbortSignal; }) => {
    ...
    const items = res?.sdtPermisos || res?.SdtPermisos || res?.items || [];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (e?.name !== "AbortError") console.error("Error cargando permisos:", e);
      }
    })();
    return () => ac.abort();
  ...
  const columns: any[] = [
    { accessorKey: "PermisoNombre", header: "Permiso" },
    { accessorKey: "PermisoDescripcion", header: "Descripción" },
    { accessorKey: "PermisoObjeto", header: "Objeto" },
    {
      accessorKey: "PermisoEstado",
      header: "Estado",
      cell: ({ row }: { row: { original: any } }) => {
        const val = String(row.original?.PermisoEstado ?? "").toUpperCase();
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: any } }) => (
        <div className="space-x-2">
          {sinMigrar && (
            <Button variant="secondary" size="sm" onClick={() => console.log("Importar permiso", row.original)}>
              <Download className="w-4 h-4" />
              <span className="ml-1">Importar</span>
            </Button>
          )}
          <Button variant="outline" size="sm" onClick={() => router.push(`/dashboard/permisos/editar/${row.original?.PermisoId || row.original?.id || ""}`)}>
            <Pencil className="w-4 h-4" />
          </Button>
          <Button variant="destructive" size="sm" onClick={() => console.log("Eliminar permiso", row.original)}>
