...
export function useAtributosRol(rolId: number, isOpen: boolean) {
  ...
  // Cargar atributos existentes del rol
  const cargarAtributos = async () => {
    if (!isOpen) return;

    setLoading(true);
    try {
      const atributosExistentes = await apiAtributosRolDB(rolId);
      ...
      console.error("Error al cargar atributos del rol:", error);
      toast.error("Error al cargar los atributos del rol");
    } finally {
      setLoading(false);
    }
  };
  ...
  const guardarAtributos = async () => {
    try {
      ...
      await apiGuardarAtributosRolDB(rolId, atributosParaDB);

      toast.success(`${atributosParaDB.length} atributo(s) guardado(s) correctamente`);

      return true;
    } catch (error) {
      console.error("Error guardando atributos del rol:", error);
      toast.error("Error al guardar atributos del rol");
      return false;
    } finally {
      setSaving(false);
    }
