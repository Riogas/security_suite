    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: any } }) => (
        <div className="space-x-2">
          {sinMigrar && (
            <Button variant="secondary" size="sm" onClick={() => console.log("Importar rol", row.original)}>
              <Download className="w-4 h-4" />
              <span className="ml-1">Importar</span>
            </Button>
          )}
          <Button variant="outline" size="sm" onClick={() => router.push(`/dashboard/roles/editar/${row.original?.RolId || row.original?.id || ""}`)}>
            <Pencil className="w-4 h-4" />
          </Button>
          <Button variant="destructive" size="sm" onClick={() => console.log("Eliminar rol", row.original)}>
