  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (e?.name !== "AbortError") console.error("Error cargando eventos:", e);
      }
    })();
    return () => ac.abort();
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: any } }) => (
        <div className="space-x-2">
          {sinMigrar && (
            <Button variant="secondary" size="sm" onClick={() => console.log("Importar evento", row.original)}>
              <Download className="w-4 h-4" />
              <span className="ml-1">Importar</span>
            </Button>
          )}
          <Button variant="outline" size="sm" onClick={() => router.push(`/dashboard/eventos/editar/${row.original?.EventoId || row.original?.id || ""}`)}>
